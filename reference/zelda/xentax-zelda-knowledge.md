# Zelda (Wii U / Switch) — XeNTaX Forum Knowledge Extraction

## 1. Breath of the Wild (Wii U / Switch) — Yaz0 / SARC / sblarc

### 1.1 Archive Pipeline
```
Game data → .pack (SARC) → .sblarc (Yaz0 compressed SARC) → content files
Path example: content/Pack/Bootup.pack/Layout/Common.sblarc
```

### 1.2 Yaz0 Compression
- **Magic**: `Yaz0` (4 bytes)
...
| Tool | Purpose | Notes |
|------|---------|-------|
| **Syroot.NintenTools.Yaz0** | C# decode/encode | https://github.com/Syroot/NintenTools.Yaz0 — NuGet package |
| **Yaz0.exe** (XeNTaX community) | CLI decode/encode | Fake compression (level 0) on encode → larger files but valid |
| **Uwizard / yaz0enc** | Community tools | Repacking issues reported (game crashes even with untouched content) |
| **Switch-Toolbox** | Multi-format | https://github.com/KillzXGaming/Switch-Toolbox — supports Yaz0, SARC, BWAV, GTX |

### 1.4 SARC Archives
- **Magic**: `SARC`
- **Tool**: JayK's unpacker (2015) — works for BotW, Mario 3D World, Captain Toad
- **Repacking**: Community asked but no confirmed working repacker in thread
...
- **File structure**: `content/Pack/*.pack` (SARC) → contains `Layout/*.sblarc` (Yaz0)
...
- **Playback**: ~90% playable in Foobar2000 with vgmstream; ~10% fail (different encoding?)

### 2.2 Tools & Resources
| Resource | Link |
|----------|------|
| GameBanana post | https://gamebanana.com/posts/10308634 |
...
- **Community work**: RandomTBush was developing a GTX splitter (2016)
...
| **OoT3D-Importer** | https://github.com/MeltyPlayer/OoT3D-Importer — Blender importer |
| **GAR - ZAR Unpack** | Extracts models/animations from ZAR |

### 5.3 CSAB Animation Structure (from shakotay2 analysis)
- Bone weights stored in `VatrChunk.BoneWeights`
- Animation data at `sepd.BoneWeightArrayOffset`
- Frame data: 16 bytes per frame (difference of offsets = count × 16)
- Skinning weights possibly in `.dae` export but not confirmed
...
| XeNTaX threads | Original discussions (see raw file) |

---

## 11. Gaps / Unresolved
- **Yaz0 maximum compression**: No public tool matches Nintendo's compression ratio
