...
- `DuniaDemo_r64_dx12.pdb` (22MB) — 266,941 type records, 2,113 publics, 31k class/struct definitions, 116k method signatures
- `DuniaDemo_r64.pdb` (19MB) — DX11 variant
- `gear_core_win64DLLR.pdb`, `gxcore_release_x64.pdb`, etc. — Gear EAL / GX layer
- `DuniaDemo_rt64.pdb` (323MB) — **corrupt** (truncated ~17%, stream dir missing); do not try to parse
...
1. `extract_dev_pdb_types.py <pdb> <dev_tag>` → `symbols/dev_types_<dev_tag>.json`,
   `dev_publics_<dev_tag>.json`, `dev_globals_<dev_tag>.json` (~21s, 100MB/type dump)
...
WDL's (no Lua natives; `globals.lua` absent → extraction auto-skips Lua).

**Enrichment yields**: WD2 178 vfN→real methods (42 classes), WDL 156 (37 classes).
All matched classes are shared SDK types (Bloomberg etc.); the engine-specific game
classes don't overlap across the version gap.
