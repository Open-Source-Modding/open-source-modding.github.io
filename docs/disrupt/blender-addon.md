...
- **WD1 `.xbgm` (XBGMIP)**: high-detail streamed LOD for vehicles. Parser exists but has no Blender export integration — parsed data is discarded. Format: 512-byte header + 2× packed vertex blocks + optional normals block. Doc: `reference/disrupt/xbgmip-format.mdx`.
