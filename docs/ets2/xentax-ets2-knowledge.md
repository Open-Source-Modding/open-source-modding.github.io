# Euro Truck Simulator 2 / American Truck Simulator (SCS Software — Prism3D Engine) — XeNTaX Forum Knowledge Extraction

## 1. Engine & Archive Overview

### 1.1 Engine
- **Engine**: **Prism3D** (SCS Software proprietary)
- **Games**: Euro Truck Simulator 2 (2012), American Truck Simulator (2016), Scania Truck Driving Simulator
- **Archive formats**: `.scs` (custom), `.zip` (mods), `.sii` (text config), `.3nk`/`.3nK` (encrypted language)

### 1.2 Archive Pipeline
```
Game data → *.scs (SCS# container) → extract → *.sii / *.pmg / *.pmd / *.mat / *.tobj / *.dds
Encrypted: *.3nk / *.3nK (locale.scs, def.scs, base_cfg.scs) → decrypt → *.sii.txt
```
...
### 2.2 Hash Algorithm
- **Custom hash**: Very long function (MASM32 implementation by Ekey)
- **Recovery method**: Unpack `def.scs` → search for paths in files (e.g., `model_desc: "/model/sign/..."`)
- **Tool**: **SCSHasher** by Ekey (0.1) — generates hashes from known paths
- **Community effort**: Collect known filenames → build hash dictionary

### 2.3 Encryption (3nK / .3nk)
...
| Tool | Status |
|------|--------|
| **QuickBMS (aluigi)** | Script supports reimport (section 3 of quickbms.txt) — but encryption reimport fails |
| **Ekey tests** | "Game don't work after reimport" (2013) — encryption callback issues |
| **aluigi fix** | QuickBMS 0.5.17c fixed reimport bug — but game still rejects reencrypted locale.scs |
...
| Tool | Author | Notes |
|------|--------|-------|
| **SCS Blender Tools** | SCS Software | Official — https://modding.scssoft.com/wiki/Documentation/Tools/Download |
| **ConverterPIXWrapper** | simon50keda | https://github.com/simon50keda/ConverterPIXWrapper — PMG/PIX → Blender |
| **3D Object Converter** | Karpati | v10.001+ supports PMG 0x11/0x13/0x15 — http://3doc.i3dconverter.com |
| **Noesis** | Durik256 / Sharppy | Scripts: `fmt_pmg.py`, `fmt_euro_truck_sim_pmg.py` |
| **SCSArchivier** | Community | Extract .scs archives |

### 4.4 Batch Conversion Pipeline (Noesis)
...
## 8. Key Repositories & Tools Summary
...
## 9. Gaps / Unresolved (from threads)
...
- **SCS modding wiki**: https://modding.scssoft.com/ — official documentation
